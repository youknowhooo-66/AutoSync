    ...
    <div className="fade-in">
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input placeholder="Pesquisar por nome ou e-mail..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', paddingLeft: '40px' }} />
        </div>
      </div>
                ...
                <tr key={user.id} style={{ borderBottom: '1px solid var(--border)', opacity: user.active ? 1 : 0.5 }}>
                  ...
                  <td style={{ padding: '1rem', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                      <button onClick={() => handleEdit(user)} style={{ background: 'transparent', border: 'none', color: 'var(--accent)', cursor: 'pointer' }}>
                        <MdEdit size={20} />
                      </button>
                      <button onClick={() => handleToggle(user)} style={{ background: 'transparent', border: 'none', color: user.active ? 'var(--warning)' : 'var(--success)', cursor: 'pointer' }}>
                        {user.active ? <MdToggleOff size={22} /> : <MdToggleOn size={22} />}
                      </button>
                    </div>
                  </td>
                </tr>
