...
export class CompleteServiceOrderUseCase {
  async execute({ serviceOrderId, companyId, operationalBranchId, userId, userRole, userBranchId, correlationId }: IRequest) {
    logger.info({ correlationId }, `[UseCase:CompleteOS] Starting execution for OS ${serviceOrderId}`);
    ...
    logger.info({ correlationId }, `[UseCase:CompleteOS] OS ${serviceOrder.number} marked as COMPLETED. Events dispatched.`);
