...
export class WorkerGuard {
  public static async validate(event: ISecureEvent, requiredPermission: Permission): Promise<void> {
    ...
    if (!companyId) {
      logger.error({ eventId: id, correlationId }, `[Security:WorkerGuard] Tenant Isolation Violation: Missing companyId`);
    ...
    if (!isAuthorized) {
      logger.error({ eventId: id, correlationId, userId, permission: requiredPermission }, `[Security:WorkerGuard] Unauthorized Worker Execution Attempt`);
