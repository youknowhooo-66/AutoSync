    ...
    <div className="fade-in">
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input placeholder="Pesquisar por nome ou CNPJ..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', paddingLeft: '40px' }} />
        </div>
      </div>
        ...
        {filtered.map((branch) => (
          <div key={branch.id} className="card" style={{ position: 'relative', borderLeft: `4px solid ${branch.active ? 'var(--accent)' : 'var(--text-muted)'}`, opacity: branch.active ? 1 : 0.6 }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
              <button onClick={() => handleEdit(branch)} style={{ background: 'rgba(56,189,248,0.1)', border: 'none', color: 'var(--accent)', cursor: 'pointer', padding: '6px 14px', borderRadius: '6px', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <MdEdit size={16} /> Editar
              </button>
              <button onClick={() => handleToggle(branch)} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: 'pointer', padding: '6px 14px', borderRadius: '6px', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                {branch.active ? <MdToggleOn size={16} /> : <MdToggleOff size={16} />}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingId ? 'Editar Filial' : 'Nova Filial'}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <input placeholder="Ex: Matriz Centro" value={name} onChange={e => setName(e.target.value)} required />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>CNPJ *</span>
            <input placeholder="00.000.000/0001-00" value={cnpj} onChange={e => setCnpj(e.target.value)} required />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            ...
            <label style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <input type="email" placeholder="email@filial.com" value={email} onChange={e => setEmail(e.target.value)} />
            </label>
          </div>
